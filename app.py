...
def connect_to_db():
    return pymysql.connect(
        host=os.environ.get("DB_HOST", "bqi7s2sh8mowissieofw-mysql.services.clever-cloud.com"),
        user=os.environ.get("DB_USER", "uilkmwbuewtgwywx"),
        password=os.environ.get("DB_PASSWORD", "YOsoRNR3HzrUicYb0Ow"),
        database=os.environ.get("DB_NAME", "bqi7s2sh8mowissieofw"),
        port=int(os.environ.get("DB_PORT", 20375)),
        cursorclass=DictCursor,
        charset="utf8mb4",
        ssl={"ssl": {}}  # fuerza conexión SSL si Clever Cloud lo requiere
    )
...
''')
    ...
    cur.execute('SELECT rol_id FROM usuarios WHERE id = %s', (usuario['id'],))
    user_data = cur.fetchone()
    
    # Update user data in session with role_id
    if user_data and 'rol_id' not in usuario:
...
def modificar(id):
    ...
    # Obtener proyecto
    cur.execute("""
        SELECT id, titulo, enlace_git, enlace_drive, archivo_pdf, nombre_archivo, Categorias,
               competencias, Autores, palabrasclave, fecha_subida, fecha_finalizacion
        FROM documentos
        WHERE id = %s
    """, (id,))
    proyecto = cur.fetchone()
    ...
    # Convertir competencias y autores de string a lista para mostrar en el input
    proyecto['competencias'] = [c.strip() for c in proyecto['competencias'].split(',')] if proyecto['competencias'] else []
    proyecto['Categorias'] = [c.strip() for c in proyecto['Categorias'].split(',')] if proyecto['Categorias'] else []
    proyecto['Autores'] = [a.strip() for a in proyecto['Autores'].split(',')] if proyecto['Autores'] else []

    if request.method == 'POST':
        # Campos básicos
        titulo = request.form['titulo']
        enlace_git = request.form['enlace_git']
        enlace_drive = request.form['enlace_drive']
        palabras_clave = request.form['palabras_clave']
        fecha_subida = request.form['fecha_subida']
        fecha_finalizacion = request.form['fecha_finalizacion']
        
        # Nuevo campo
        estado = request.form['estado']  # obtenemos el estado seleccionado

        # Campos múltiples
        categorias_seleccionadas = request.form.getlist('categorias[]')  # <select multiple>
        competencias_texto = request.form['competencias']  # input de texto
        autores_sel = request.form.getlist('autores[]')  # <select multiple>

        # Limpiar y unir competencias y categorías para guardar
        competencias_lista = [c.strip() for c in competencias_texto.split(',') if c.strip()]
        competencias_guardar = ','.join(competencias_lista)
        categorias_guardar = ','.join(categorias_seleccionadas)
        autores_guardar = ','.join(autores_sel)
        ...
        cur.execute("""
            UPDATE documentos
            SET titulo=%s, enlace_git=%s, enlace_drive=%s,
                archivo_pdf=%s, nombre_archivo=%s,
                Categorias=%s, competencias=%s, Autores=%s,
                palabrasclave=%s, fecha_subida=%s, fecha_finalizacion=%s, estado=%s
            WHERE id=%s
        """, (
            titulo, enlace_git, enlace_drive,
            archivo_pdf, nombre_archivo,
            categorias_guardar, competencias_guardar, autores_guardar,
            palabras_clave, fecha_subida, fecha_finalizacion, estado, id
        ))
        conn.commit()
        cur.close()
        conn.close()
    ...
    return render_template('modificar.html', proyecto=proyecto, categorias=categorias, autores=autores)
...
def debug_session():
    """Ruta de depuración para ver los datos de sesión"""
    if 'usuario' not in session:
        return 'No hay usuario en la sesión', 200
    ...
    <h2>Session Data:</h2>
    <pre>{session}</pre>
    <h2>User Data from DB:</h2>
    <pre>{user_data}</pre>
    <h2>Session Usuario:</h2>
    <pre>{session.get('usuario')}</pre>
    '''
